...
output "dns_zone_records" {
  ...
  value = merge(
    ...
    { for k, v in azapi_resource.caa : k => { id = v.id, name = v.name, fqdn = try(v.output.fqdn, null), type = "CAA" } },
    ...
    { for k, v in azapi_resource.ns : k => { id = v.id, name = v.name, fqdn = try(v.output.fqdn, null), type = "NS" } },
...
output "dns_zone_records_by_type" {
  ...
  value = {
    ...
    caa   = { for k, v in azapi_resource.caa : k => { id = v.id, name = v.name, fqdn = try(v.output.fqdn, null) } }
    ...
    ns    = { for k, v in azapi_resource.ns : k => { id = v.id, name = v.name, fqdn = try(v.output.fqdn, null) } }
