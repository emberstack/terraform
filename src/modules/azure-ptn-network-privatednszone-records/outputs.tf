...
output "private_dns_zone_records_by_type" {
  description = "Records grouped by type, each value a map of `{fqdn, resource_id, name}` keyed by the input map key. Group keys use ARM's own casing (`A`, `AAAA`, `CNAME`, ...), matching the `type` field of `private_dns_zone_records`, so the two outputs compose."
  value = {
